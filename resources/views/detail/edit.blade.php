                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('detail.update', $detail->id) }}" method="POST">
                            ...
                            <div class="col-sm-10">
                                <select name="produk_id" class="form-control" id="produk_id">
                                    ...
                                    @foreach($produk as $dp)
                                        <option value="{{ $dp->id }}" {{ ($dp->id == $detail->produk_id) ? ' selected' : '' }}>{{ $dp->nama_produk }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-sm-10">
                                <input type="number" name="jumlah_produk" value="{{ old('jumlah_produk', $detail->jumlah_produk) }}"
